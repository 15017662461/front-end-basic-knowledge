...
5. HTTPS证书如何保证不被篡改？数字签名。数字签名就是对元信息(证书所有者的信息，例如基本信息、公钥等)。然后使用CA机构的公钥进行解密验证元信息是否一致就能判断证书是否合法
6. 前端加密：md5加密
...
Array.prototype.myFlat = function(count){
  ...
  return arr.reduce((res,value) => {
    if(Array.isArray(value)){
      res = res.concat(value.myFlat(count-1))
    }else{
...
function getRandomArr(arr){
  for(let i = 0;i < arr.length;i ++){
    const random = Math.floor(Math.random()*(arr.length - i))
    let temp = arr[random]
    arr[random] = arr[arr.length - i - 1]
  ...
  start(){
    if(this.count >= this.tasks.length){
      ...
      for(let i = 0;i <this.count;i ++){
        this.working.push(this.tasks.shift())
      }
      this.working.forEach(task => task().then(() => {
        this.next(task)
      }))
    }
  }
...
function rightSideView(root){
  ...
  while(queue1.length || queue2.length){
    while(queue1.length){
      ...
      if(!queue1.length) res.push(temp.val)
    }
    queue1 = queue2
    queue2 = []
  }
