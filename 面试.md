...
+ Set:成员是唯一的不能重复，可以遍历，常用方法有：add/has/delete/clear
+ WeakSet: 在Set的基础上还保证了成员都是对象，成员只是弱引用，可以被垃圾回收机制回收，常用来保存Dom节点(当dom节点被删除时Set中也被删除)，不能被遍历
+ Map：键值对的集合，可以被遍历，常用方法有：set/has/delete/get/clear
...
prototype是函数才有的属性，比如Object、Array和构造函数等，它们都有着prototype，它们的prototype.cunstructor就是自己本身：
...
构造函数通过new 出来的实例对象的\__proto__就是构造函数的prototype
...
2. 攻击分类：①持久型XSS，主动提交代码到服务器，用户请求后，服务器将恶意代码发给用户   ②非持久型XSS，诱骗用户点击URL带攻击代码的链接，服务器解析后响应，在响应内容中嵌入攻击者的XSS代码。③dom XSS，根据实际情况构造dom节点进行XSS跨站脚本攻击
...
3. 防范：①敏感请求不采用get形式（只能防范一些CSRF攻击）②用户操作限制，添加验证码机制（简单粗暴，但对用户不太友好）③验证http referer字段，这个字段记录了请求的来源地址，服务器验证来源地址是否合法（依赖浏览器）④采用token 。

### 浏览器垃圾回收机制

**js引擎使用的是标记清除**

+ 标记清除(js引擎使用的垃圾回收机制)。当变量进入环境时，将变量标记为进入环境，离开环境时标记为离开环境。某一时刻，垃圾回收机制会过滤掉环境中的变量以及被环境变量引用的变量，剩下的就是被准备回收的变量
...
+ created ：  实例创建完成之后被调用。这一步已经完成了数据观测，属性计算和时间绑定
...
+ deactivated ：kepp-alive组件停用时调用
+ beforeDestroy ： 实例销毁前调用，这里实例仍然可用
+ destroyed：实力完全销毁后，所有事件等都被移除绑定
...
babel的作用：将ES的高版本转换成适合浏览器运行支持的低版本
...
+ + 301 Moved Permanently  永久重定向，请求的资源已经不存在了，需要使用新的URL，在响应头中会有Location字段，指明后续要跳转的URL，浏览器会自动重定向过去
...
5. **缺点：**虽然有管道机制，但服务器回应时依旧可能造成队头堵塞。为了解决这一问题，只有两种办法：减少请求数和同时多开持久连接
...
4. HTTPS协议需要像CA（证书权威机构）申请数字证书来保证服务器的身份是可信的，服务器的公钥就放在数字证书中。

#### HTTP缓存

http缓存分为强制缓存和协议缓存

1. 强制缓存：通过Expires、Cache-Control、Pragma控制，三者同时出现时：pragma > cache-control > expires。浏览器在发送请求之前会判断当前的时间是否在Cache-Control：max-age内
...
3. 对于前端，可以在入口页面使用no-cache或max-age设置为较短时间，js.css本身设置max-age较长时间，引用js、css通过打包工具自动加上hash版本号的方式，这样浏览器指挥请求修改的文件。
...
**七层网络模型：**物理层、数据链路层、网络层、传输层、表示层、会话层、应用层

**四层网络模型：**数据链路层、网络层。传输层、应用层、
