...
1. React的组件文件最后不能使用ReactDOM.render进行渲染，应该使用module.exports = <组件 />返回。并且，组件内的导入方式要改为require的形式。
2. 在服务端的node中，要注意对代码window进行hack，替换掉来避免报错
3. webpack配置中，需要对相应的文件名进行处理
...
+ extractTextWebpackPlugin：将CSS从bundle中提取成一个单独的CSS
...
plugin是一个函数或者是一个包含apply方法的对象。传递给插件的compiler暴露了webpack 的整个生命周期钩子，compilation暴露了和模块依赖有关的粒度更小的事件钩子
