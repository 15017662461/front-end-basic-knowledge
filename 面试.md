...
主要分为以下五个步骤：查找强缓存、DNS域名解析、建立TCP连接、发送HTTP请求、关闭TCP连接、浏览器渲染。会先在本地查找是否有强缓存，如果有且没过期就直接显示
...
在登录时发送请求到sso.a.com，登录完成后，sso.a.com设置cookie的domain属性为顶级域也就是a.com。这样后续的操作cookie也会被带上方位app1和app2.session课已使用共享session的解决方案例如Spring-Session
