...
1. history：history.pushState()、history.replaceState()  这两种方式都会引起URL的变化但页面不会刷新。通过监听popState事件可以感知到URL的变化从而改变组件的展示
...
需要安装url-loader和file-loader，其中url-loader用于将小的图片文件直接base64编码，大的图片交给file-loader处理。file-loader处理字体图片等文件，在代码中通过相对URL去引用输出的文件：如果单独用file-loader则不会对小图片进行base64。使用file-loader会自动将超过limit大小的使用file-loader。所以，一般推荐直接使用url-loader即可
...
在开启热更新之后，本地会开启一个Bundle Server提供localhost服务器的访问方式，构建生成的bundle.js文件中会被注入一个HMR Runtime当接收/监听到文件改变后会将文件更新。同时，本地会开启一个HMR Server负责将热更新的文件通过websocket的方式输出给HMR Runtime
...
由于hash是在生成环境中的，而热更新是在开发环境中的，所以下面对config文件进行重构，将上面的文件重命名为webpack.dev.js。复制一份为webpack.prod.js。修改package.json文件的scripts如下：
...
1. 将html内容内联到另一个html中。例如需要将大量的meta标签内联进来。使用到了raw-loader。在需要内联的地方写上：

   ```html
   <%= require('raw-loader!./meta.html') %>
   ```

2. 将js代码内联到html中，也使用到了raw-loader

   ```html
   <script><%= require('raw-loader!babel-loader!../node_modules/lib-flexible/flexible.js') %></script>
   ```

3. css内联到html中，这有两种方式，一种是使用style-loader插入到style标签内。第二种方式是使用html-inline-css-webpack-plugin
...
概念：一个模块可能包含了多种方法，但只要其中某个方法被使用到了，整个文件就会被引入到bundle中去，这是非常浪费的。于是有了tree shaking。没用到的方法会在uglify阶段被擦除掉。
...
module.exports = {
  ...
  optimization:{
    minimize:true,
    minimizer:[
      new TerserPlugin({ // 设置压缩，排除.min.js文件不进行压缩
        include:/\.min\.js$/
      })
    ]
  }
...
+ 预编译资源模块。使用DLLPlugin进行分包，DLLReferencePlugin对manifest.json引用。简单来说就是将项目中的react、react-dom、redux等基础包和业务基础包打包成一个文件。使用方式：先创建一个webpack.dll.js文件，关于dllplugin的配置；再在webpack.config.js中使用DLLReferencePlugin引用manifest.json。然后在package.json中新建一个dll命令运行webpack.dll.js，运行dll之后再进行build
...
简单抽象一下核心过程：![img](https://user-gold-cdn.xitu.io/2020/7/29/1739822cc61ee5fa?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
webpack会按照从右到左的顺序链式调用每一个loader，所以每一个loader的返回值必须是标准的js代码字符串以保证下一个调用的loader能够正常工作，同时在开发上需要严格遵循单一职责，只关心loader的输出以及对应的输出。

loader函数中的this由webpack提供，可以通过this对象提供的相关属性，获取当前loader需要的各种信息数据。this指向的是loaderContext对象

需求：手写loader，处理.txt文件，对字符串做翻转并将翻转后的首字母大写。例如abcdefg转换后为Gfedcba。按照上面说的单一职责原则，我们需要写两个loader，第一个reverse-loader负责翻转，第二个uppercase-loader负责大写。
...
plugin是一个函数或者是一个包含apply方法的class类。传递给插件的compiler暴露了webpack 的整个生命周期钩子，compilation暴露了和模块依赖有关的粒度更小的事件钩子

需求：在static/css/pages下的css文件导入的static/css/vendor.wxss文件路径前全部加上/subPages/enjoy_given/，也就是将@import "/static/css/vendor.wxss"修改为 @import "/subPages/enjoy_given/static/css/vendor.wxss"
    ...
    compiler.plugin('emit',(compilation,callback) => {
      for(let filePathName in compilation.assets){
        if(/static\/css\/pages/i.test(filePathName)){
          // 路径正则，符合的需要被替换
          const reg = /\/static\/css\/verdor\.wxss/i
          // 替换后的结果
...
```js
var CssPathTransfor = require('../CssPathTransfor.js')
module.exports = {
  entry: {
    index: './src/js/index.js'
  },
...
### 手写一个简单的webpack打包js文件

这个简易版的webpack主要包含下面的功能：

1. 读取入口文件，收集依赖信息
2. 递归读取所有依赖模块，产出完整的依赖列表
3. 将各模块内容打包成一块完整的可运行代码

需要安装的依赖有：

+ @babel/parser: 解析代码生成AST
+ @babel/traverse: 遍历AST，找出依赖列表
+ @babel/core: 将修改后的AST转换成新的代码并输出

下面开始正式手写简易版webpack：

一、创建一个入口文件myPack.js并引入依赖。对某一个模块进行解析，生成其模块信息，包括模块路径、模块依赖、模块转换后的代码
...
三、经过第二步得到了依赖树，描述整个应用的依赖情况，最后我们只需要按照目标格式进行打包输出即可
...
+ + 尽量使用三维transform，会开启硬件加速 。使用will-change采用鄂城线程处理动画提高渲染效率
